  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        console.error('Failed to submit volunteer application:', error.message);
        if (error.code === 'P2000' && error.meta && error.meta.column_name){
          console.error('Column causing the issue:', error.meta.column_name)
        }
      } else {
