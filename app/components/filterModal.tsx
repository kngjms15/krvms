            ...
            <div className="justify-start flex-1 mb-4 mt-3">
                {filterOptions.filterBy.map((option,index)=> (
                    <div key={`A${index}`} className={`rounded-lg p-1 pr-4 ${selectedFilterOption===option && 'bg-gray-600 text-white'}`}>
                        <input type="radio" id={`${selectedFilterOption===option?`${filterName}SelectedA`:`${filterName}UnselectedA${index}`}`} 
                        value={option} checked={selectedFilterOption===option} onChange={()=>{handleSelectFilter(option)}} className="selected:bg-white"/>&nbsp;&nbsp;{option}
                    </div>
                ))}
            </div>
